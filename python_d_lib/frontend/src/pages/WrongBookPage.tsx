...
import { BookMarked, Calculator, Languages, Trash2 } from "lucide-react";
...
function WrongBookRow({ item, onDismiss }: { item: WrongBookItem; onDismiss: (id: string) => void }) {
    ...
    <li className="rounded-2xl border border-white/70 bg-white/90 p-4 shadow-sm ring-1 ring-primary/10 transition hover:shadow-md">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <button
          ...
          className="inline-flex shrink-0 items-center gap-1 rounded-lg border border-black/[0.08] bg-white px-2.5 py-1.5 text-caption font-semibold text-ink-muted transition hover:border-red-200 hover:bg-red-50 hover:text-red-700"
...
export function WrongBookPage() {
  ...
  const [items, setItems] = useState(() => loadWrongBookItems());

  const refresh = useCallback(() => setItems(loadWrongBookItems()), []);
