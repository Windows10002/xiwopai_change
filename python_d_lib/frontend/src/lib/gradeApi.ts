...
function normalizeNumericAnswerToken(s: string): string | null {
...
/**
 * 若模型将「正确/过程不规范」判给最终数值明显不符的题目，前端纠为错误并压低总分（保留过程分）。
 */
function reconcileMathQuestions(questions: Array<Record<string, unknown>>): Array<Record<string, unknown>> {
  return questions.map((q) => {
    ...
    if (st !== "正确" && st !== "过程不规范") return q;
    const exp = String(q.expected_answer ?? "").trim();
    const got = String(q.student_answer ?? "").trim();
    if (!exp || !got) return q;
    const ne = normalizeNumericAnswerToken(exp);
    const ng = normalizeNumericAnswerToken(got);
    if (!ne || !ng || ne === ng) return q;
    const psRaw = Number(q.process_score ?? Number.NaN);
    const processPart = Number.isFinite(psRaw) ? clampNum(Math.max(psRaw, 4), 0, 6) : 5;
    const struct = Number(q.structure_score ?? 0);
    const oldReason = String(q.reason ?? "").trim();
    const tailHint = "（订正提示：末步结果与标准答案不一致，已按规则计为错误并保留部分过程分。）";
    return {
      ...q,
      status: "错误",
      result_score: 0,
      process_score: processPart,
      total_score: processPart + struct,
      reason: oldReason ? `${oldReason}\n${tailHint}` : tailHint,
    };
  });
}
...
export function mapMathResult(raw: Record<string, unknown>): GradingResultDetail {
  ...
  questions.forEach((q, i) => {
    ...
    if (st === "正确") {
      const expr = String(q.expr ?? "本题");
      strengths.push(buildMathStrengthLine(id, expr, reasonRaw));
      return;
    }
    if (!reason) return;
...
export function mapEnglishResult(raw: Record<string, unknown>): GradingResultDetail {
  ...
  const improvements =
    errors.length > 0
      ? errors.slice(0, 16).map((e) => {
          const tag = String(e.tag ?? "").trim();
          const orig = truncate(String(e.original ?? ""), 80);
          const fix = truncate(String(e.correction ?? ""), 80);
          const tail = [orig && `原：${orig}`, fix && `改：${fix}`].filter(Boolean).join("；");
          return tag ? `${tag} ${tail}` : tail || "请查看错误订正条目";
        })
      : weak.map((w) => `薄弱方向：${w}`);
