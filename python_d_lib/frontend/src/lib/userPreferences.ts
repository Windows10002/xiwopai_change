...
const LEGACY_KEY = "seewo_pi_user_prefs_v1";
...
/** 从 v1 迁移：曾用 showGradingFabHelp 控制批改页帮助 */
function loadLegacyV1(): Partial<UserPreferences> | null {
  try {
    ...
    return {
      ...
      largerText: o.largerText,
      showGradingFabHelp: o.showGradingFabHelp,
      showHomeFabHelp: true,
    };
  } catch {
    return null;
...
export function loadUserPreferences(): UserPreferences {
  try {
    ...
    if (!raw) {
      ...
      if (legacy) {
        const merged = normalizePartial({ ...DEFAULT_USER_PREFERENCES, ...legacy });
